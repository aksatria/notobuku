...
class FilsafatBukuProgresifSeeder extends Seeder
{
    public function run(): void
    {
        $institutionId = (int) (DB::table('institutions')->value('id') ?? 1);
        $branchId = $this->resolveSurabayaBranchId($institutionId);
        $metadata = app(MetadataMappingService::class);

        $productUrls = $this->collectProductUrls();
        if (empty($productUrls)) {
            $this->command?->warn('Tidak ada URL produk filsafat yang ditemukan pada halaman 1-3.');
            return;
        }
        ...
        foreach ($productUrls as $url) {
            ...
            $this->ensureSurabayaItem($biblio, $institutionId, $branchId);
            ...
            $metadata->syncMetadataForBiblio(
                ...
                [
                    'id' => [
                        ...
                        'subject' => array_values(array_unique(array_merge(['Filsafat'], $categories, $tags))),
    ...
    {
        $pages = [
            'https://bukuprogresif.com/product-category/filsafat/',
            'https://bukuprogresif.com/product-category/filsafat/page/2/',
            'https://bukuprogresif.com/product-category/filsafat/page/3/',
        ];
        ...
        foreach ($pages as $pageUrl) {
            ...
            preg_match_all('/href=["\'](https:\/\/bukuprogresif\.com\/product\/[^"\']+)["\']/i', $html, $m);
            foreach (($m[1] ?? []) as $u) {
                $clean = strtok($u, '?');
    ...
    private function resolveSurabayaBranchId(int $institutionId): ?int
    {
        $byCode = Branch::query()
    ...
    {
        $subjects = array_values(array_unique(array_merge(['Filsafat'], $categories, $tags)));
        if (empty($subjects)) {
            return;
        }
    ...
    {
        $tags = array_values(array_unique(array_merge(['bukuprogresif', 'filsafat'], $sourceTags)));
    ...
    private function ensureSurabayaItem(Biblio $biblio, int $institutionId, ?int $branchId): void
    {
        if ($branchId === null) {
            return;
        }
    ...
    private function inferDdc(array $categories, array $tags): ?string
    {
        ...
        if (str_contains($text, 'filsafat') || $text === '') {
            return '100';
        }
        return '100';
    }
    ...
    private function downloadCover(string $url, string $title): ?string
    {
        try {
            ...
            $file = 'covers/bukuprogresif-filsafat-' . Str::slug($title) . '-' . Str::lower(Str::random(6)) . '.' . $ext;
