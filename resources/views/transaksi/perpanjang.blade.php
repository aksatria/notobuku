...
@section('title', 'Transaksi Perpanjang • NOTOBUKU')

@section('content')
@php
  $title = $title ?? 'Transaksi Perpanjang';
@endphp
...
<style>
  ...
  .nb-k-badges{ display:flex; gap:8px; flex-wrap:wrap; margin-top:8px; }
  .nb-k-badge{
    display:inline-flex; align-items:center; gap:6px;
    padding:6px 10px; border-radius:999px; font-size:12px; font-weight:700;
    border:1px solid rgba(15,23,42,.12); color: rgba(11,37,69,.85);
    background: rgba(255,255,255,.75);
  }
  html.dark .nb-k-badge{
    border-color: rgba(148,163,184,.16);
    background: rgba(15,23,42,.45);
    color: rgba(226,232,240,.92);
  }
  .nb-k-badge.info{ border-color: rgba(30,136,229,.18); color:#0B3B7A; background: rgba(30,136,229,.08); }
  html.dark .nb-k-badge.info{ color: rgba(191,219,254,.95); background: rgba(30,136,229,.18); }
  ...
  .tx-col-barcode{ width: 26%; }
  .tx-col-loan{ width: 18%; }
  .tx-col-due{ width: 18%; }
  .tx-col-newdue{ width: 18%; }
  .tx-col-renew{ width: 10%; text-align:center; }
...
<div class="nb-k-wrap">
  <div class="nb-card nb-k-head">
    ...
    <form method="POST" action="{{ route('transaksi.perpanjang.store') }}" id="form-perpanjang">
      @csrf
                ...
                <div class="tx-tableWrap">
                  <table class="nb-table">
                    ...
                    <tbody id="items_table">
                      <tr id="items_empty">
                        <td colspan="7" class="nb-muted" style="padding:14px 14px;">
                          <div class="tx-empty">
                            <div class="ico">🗓️</div>
                ...
                <div class="tx-footer">
                  <div style="display:flex; justify-content:space-between; gap:10px; flex-wrap:wrap; align-items:center;">
                    ...
                    <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                      ...
                      <button type="submit" class="nb-k-ibtn apply" title="Simpan perpanjang" aria-label="Simpan perpanjang">
...
(function(){
  ...
  const inputsWrap = document.getElementById('renew_inputs');
  const countInfo  = document.getElementById('count_info');

  const form = document.getElementById('form-perpanjang');
  const maxRenewals = {{ (int) config('notobuku.loans.max_renewals', 2) }};
  ...
  async function addBarcode(){
    ...
    const url = "{{ route('transaksi.perpanjang.cek_barcode') }}?barcode=" + encodeURIComponent(barcode);
    ...
    if(!loanItemId){
      alert('Data perpanjang tidak valid (loan_item_id kosong).');
      barcodeInput.focus();
      return;
    }

    if(added.has(loanItemId)){
      alert('Item ini sudah ada di daftar perpanjang.');
      barcodeInput.value = '';
      barcodeInput.focus();
      return;
    }
    ...
    const dueOld = formatDateTime(data.due_old ?? data.due_at ?? null);
    const dueNew = formatDateTime(data.due_new ?? data.new_due_at ?? null);
    const renewCount = Number(data.renew_count ?? 0);
    const renewWarn = renewCount >= Math.max(0, maxRenewals - 1);

    let cls = 'sb ok';
    let label = 'Dipinjam';
    const statusRaw = String(data.status ?? '').toLowerCase();
    if(statusRaw.includes('overdue')){ cls = 'sb bad'; label = 'Terlambat'; }
    else if(statusRaw.includes('borrow')){ cls = 'sb ok'; label = 'Dipinjam'; }
    else if(statusRaw){ cls = 'sb warn'; label = String(data.status); }
    ...
    tr.innerHTML = `
      ...
      <td>${escapeHtml(dueOld)}</td>
      <td>${escapeHtml(dueNew)}</td>
      <td style="text-align:center;">
        <span class="tx-renew ${renewWarn ? 'warn' : ''}">
