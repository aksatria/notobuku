...
<div class="saas-page">
  <div class="saas-card">
    <div class="saas-head">
      <div>
        <h1 class="saas-title">Edit Vendor</h1>
        <div class="saas-sub">Perbarui data vendor.</div>
      </div>
      <a class="nb-btn" href="{{ route('acquisitions.vendors.index') }}">Kembali</a>
    </div>

    <form method="POST" action="{{ route('acquisitions.vendors.update', $vendor->id) }}" style="margin-top:12px;">
      @csrf
      @method('PUT')
      <div class="field" style="margin-bottom:10px;">
        <label>Nama</label>
        <input class="nb-field" name="name" value="{{ $vendor->name }}" required>
      </div>
      <div class="field" style="margin-bottom:10px;">
        <label>Catatan</label>
        <textarea class="nb-field" name="notes" rows="3">{{ $vendor->notes }}</textarea>
      </div>
