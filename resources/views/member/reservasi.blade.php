{{-- resources/views/member/reservasi.blade.php --}}
@extends('layouts.member')

@section('title','Reservasi Saya • NOTOBUKU')
@section('member_title','Reservasi Saya')
@section('member_subtitle','Reservasi yang kamu buat dari katalog.')
...
@php
  ...
  $statusInfo = function($r){
    $status = (string)($r->status ?? 'pending');
    return match($status){
      'fulfilled' => ['label'=>'Dipenuhi','pill'=>'green','tint'=>'tint-green'],
      'cancelled' => ['label'=>'Dibatalkan','pill'=>'red','tint'=>'tint-orange'],
      default     => ['label'=>'Menunggu','pill'=>'blue','tint'=>'tint-blue'],
    };
  };

  $titleOf = function($r){
    return $r->title ?? $r->biblio_title ?? $r->book_title ?? '—';
  };
@endphp
...
<div class="nb-wrap">
  <div class="nb-panel">
    ...
    <div class="nb-section">
      <div class="nb-toolbar">
        <div style="min-width:0;">
          <div class="nb-h2">Daftar Reservasi</div>
          <div class="nb-hint">
            {{ method_exists($reservations,'total') ? $reservations->total() : $reservations->count() }} reservasi
          </div>
        </div>
    ...
    @elseif($reservations->isEmpty())
      <div class="nb-section">
        <div class="nb-h2">Belum ada reservasi</div>
        <div class="nb-hint">Buat reservasi dari halaman detail buku di katalog.</div>
      </div>
      ...
      <div class="nb-section">
        <div class="nb-grid">
          @foreach($reservations as $r)
            @php
              $info = $statusInfo($r);
              $made = $fmt($r->created_at ?? null);
              $title = $titleOf($r);
              $canCancel = in_array((string)($r->status ?? 'pending'), ['pending','waiting'], true);
              $pickup = $fmt($r->pickup_until ?? null);
            @endphp
            ...
            <div class="nb-card {{ $info['tint'] }}">
              ...
              <div class="nb-actions-row">
                <a class="nb-btn" href="{{ route('katalog.show', $r->biblio_id ?? $r->biblioId ?? $r->book_id ?? 0) }}">Lihat Buku</a>

                @if($canCancel)
                  <form method="POST" action="{{ route('member.reservasi.cancel', $r->id) }}">
                    @csrf
                    <button type="submit" class="nb-btn danger" onclick="return confirm('Batalkan reservasi ini?')">Batalkan</button>
                  </form>
                @endif
              </div>
            </div>
    ...
    <div class="nb-section">
      <div class="nb-hint">
        Catatan: Status & batas waktu ambil mengikuti aturan cabang. Jika buku sudah tersedia, segera ambil agar tidak hangus.
      </div>
    </div>
  </div>
</div>
